...
### Q: 如何让机制解释"让人通透"？（纠偏→静动分离→时序→口诀）

当报告需要解释一个**容易混淆的机制**（如中间件、状态流转、动态提示词注入），按以下 7 步展开：

| # | 步骤 | 示例 |
|---|------|------|
| ① | **截取典型误解** | "你可能以为中间件是最后一次调模型前跑..." |
| ② | **点出精确缺口** | "方向对，但缺了两个点：是每次调模型前跑；注入的是摘要不是全文" |
| ③ | **静态⇄动态分离** | 用 `═══ 静态准备（启动时一次）═══` 和 `═══ 动态执行（每次请求）═══` 分段 |
| ④ | **带锚点的时序图** | ASCII 流程图，每次关键触发点标注 `← 注意！每次调模型都触发` |
| ⑤ | **纠偏对照表** | `| 你的理解 | 精确表述 |` |
| ⑥ | **一行口诀** | "静态准备三个东西，动态执行两个阶段" |
| ⑦ | **告知掌握深度** | "记住时序图 + 口诀即可。TypedDict vs Pydantic 是实现细节，用的时候再查" |

> 这个模式**不替代** MDA——而是嵌入在 MDA 的"理论说明"层内部使用。MDA 是章节骨架，纠偏模式是单节内部的展开方式。
